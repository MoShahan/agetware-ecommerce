...
const Products = () => {
  const [productsList, setProductsList] = useState<EachProduct[]>([]);
  const [modifiedProductsList, setModifiedProductsList] = useState<
    EachProduct[]
  >([]);
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        ...
        const formattedData = res.data?.products.map(
          (product: EachProduct) => ({
            id: product?.id,
            title: product?.title,
    ...
    <Box id="products" sx={{ px: 4, py: 2 }}>
      ...
      <Box sx={ProductsWrapper}>
        {loading ? (
          ...
          <>
            {modifiedProductsList?.map((product: EachProduct) => (
              <ProductsCard
                key={product?.id}
                product={product}
